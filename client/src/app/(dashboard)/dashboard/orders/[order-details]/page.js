...
export default function OrderDetails() {
  ...
  async function extractOrderDetails() {
    try {
      ...
      const { data } = await api("/orders/" + params["order-details"]);
  ...
  const formattedDatetime = (currentDatetime) =>
    currentDatetime &&
    new Date(currentDatetime).toLocaleString("fr", {
      year: "numeric",
      month: "2-digit",
      day: "2-digit",
      hour: "2-digit",
    ...
    <div className="py-14 px-4 md:px-6">
      <div className="flex justify-start items-start space-y-2 flex-col">
        <h1 className="text-3xl lg:text-4xl font-bold leading-7 lg:leading-9 text-gray-900">
          Order #{orderDetails && orderDetails._id.$oid}
        </h1>
        <p className="text-base font-medium leadong-6 text-gray-600">
          {formattedDatetime(orderDetails.createdAt)}
        </p>
      </div>
            ...
            orderDetails.orderItems.length
              ? orderDetails.orderItems.map((item) => (
                  ...
                  >
                    ...
                    <div className="border-b border-gray-300 md:flex-row flex-col flex justify-between items-start w-full pb-8 md:space-y-0">
                      <div className="w-full flex flex-col justify-start items-start space-y-2">
                        <h3 className="text-xl font-semibold leading-6 text-gray-900">
                          {item && item.product.name}
                        </h3>
                        <span className="text-sm leading-6 text-gray-900">
                          Qty {item && item.product && item.product.qty}
                        </span>
                      ...
                      <div className="w-full flex items-start space-x-2">
                        <h3 className="text-xl font-semibold leading-6 text-gray-900">
                          {item &&
                            item.product &&
                            number(item.product.price * item.product.qty)}{" "}
                          DT
                        </h3>
                        {item.product.qty > 1 && (
                          <h3 className="text-sm leading-6 text-gray-900">
                            (
                            {item && item.product && number(item.product.price)}{" "}
                            DT each)
                          </h3>
                        )}
                      </div>
              ...
              <div className="flex justify-center items-center w-full space-y-4 flex-col border-gray-200 border-b pb-4">
                <div className="flex justify-between w-full">
                  ...
                  <p className="text-base leading-5 text-gray-900">
                    {orderDetails && number(total)} DT
                  </p>
                </div>
                <div className="flex justify-between w-full">
                  <p className="text-base leading-5 text-gray-800">Shipping</p>
                  <p className="text-base leading-5 text-gray-900">7 DT</p>
                </div>
                <div className="flex justify-between w-full">
                  <p className="text-base leading-5 text-gray-800">Subtotal</p>
                  <p className="text-base leading-5 text-gray-900">
                    {orderDetails && number(total + 7)} DT
                  </p>
                </div>
              </div>
            </div>
